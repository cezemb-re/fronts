...
function parseEnv(): Env {
  ...
  const windowEnv: NativeEnv =
    'env' in window && window.env ? (window.env as NativeEnv) : {};

  const env: Env = {};

  if (
    'RELEASE_VERSION' in windowEnv &&
    windowEnv.RELEASE_VERSION !== undefined
  ) {
  ...
  if (
    'NODE_APP_INSTANCE' in windowEnv &&
    windowEnv.NODE_APP_INSTANCE !== undefined
  ) {
    ...
    env.NODE_APP_INSTANCE = parseInt(
      processEnv.REACT_APP_NODE_APP_INSTANCE,
      10
    );
  }
  ...
  if (
    'DEFAULT_TIME_ZONE' in windowEnv &&
    windowEnv.DEFAULT_TIME_ZONE !== undefined
  ) {
  ...
  if (
    'DEFAULT_REGION_CODE' in windowEnv &&
    windowEnv.DEFAULT_REGION_CODE !== undefined
  ) {
  ...
  if (
    'DEFAULT_CURRENCY' in windowEnv &&
    windowEnv.DEFAULT_CURRENCY !== undefined
  ) {
  ...
  if ('API_HOST' in windowEnv && windowEnv.API_HOST !== undefined) {
    ...
    'REACT_APP_API_HOST' in processEnv &&
    processEnv.REACT_APP_API_HOST !== undefined
  ) {
  ...
  if ('API_KEY' in windowEnv && windowEnv.API_KEY !== undefined) {
    ...
    'REACT_APP_API_KEY' in processEnv &&
    processEnv.REACT_APP_API_KEY !== undefined
  ) {
